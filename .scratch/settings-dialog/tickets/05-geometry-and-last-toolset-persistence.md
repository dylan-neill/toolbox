...
- [x] On launch, saved window geometry and `last_toolset` are restored — falling
...
- [x] Window geometry is saved on window close (base64 of `saveGeometry()`);
  `last_toolset` is saved when the user changes the toolset.
- [x] The on-change save is guarded (signal-block / re-entrancy flag) so
  programmatic combo updates (reload, launch restore) do not persist spurious
  values — only genuine user changes write.
- [x] Both saves use load-modify-write so `config_path`/`terminal` settings are
  never clobbered.
